...
$$
O_t \equiv R^{\mathrm{HF}}_t - \hat\beta\, R^{\mathrm{bank}}_t \;\xrightarrow{\;p\;}\; (1-w)\,R^{\mathrm{other}}_t,
\tag{10'}
$$

the empirical counterpart of $\alpha + \boldsymbol{\delta}^\top\mathbf{F}_t + \varepsilon_t$ in (5) — i.e. the off-bank slice's return scaled by its AUM share $(1-w)$. Its left side is **observed**; its right side is the factor attribution:

$$
\underbrace{R^{\mathrm{HF}}_t - \hat\beta\, R^{\mathrm{bank}}_t}_{O_t \text{ (observed)}} = \underbrace{\hat\alpha}_{\text{off-bank drift}} + \underbrace{\sum_{k=1}^{K}\hat\delta_k F_{k,t}}_{\text{off-bank factor exposure}} + \underbrace{\hat\varepsilon_t}_{\text{idiosyncratic}}.
\tag{10''}
$$

Here $\hat\delta_k \to (1-w)\gamma_k$ is the factor-$k$ exposure the fund carries that the slice does not; $\hat\alpha$ is off-bank drift (carry, away-book skill, fees); $\hat\varepsilon_t$ is the idiosyncratic off-bank return. **Two caveats:** (i) $O_t$ inherits any bias in $\hat\beta$ — an attenuated $\hat\beta$ (synthetic-slice case) leaves residual bank exposure in $O_t$, so use actual $R^{\mathrm{bank}}_t$; (ii) the $(1-w)$ scaling is not separately identified, so a large $\hat\delta_k$ may mean a big off-bank book with modest tilts or a small one with aggressive tilts.

### 6.2 Actual vs. synthetic slice: effect on the decomposition

The factor pieces $S_{k,t-1}/A^{\mathrm{bank}}_{t-1}$ in (10) come from the sensitivity vector $\mathbf{S}_{t-1}$ regardless of which slice return enters Stage 1. *What changes is the scalar $\hat\beta$ that multiplies them, and where the slice's non-factor P&L $\eta_t$ is booked.* Let $\kappa \equiv \operatorname{Var}(R^{\mathrm{bank}})/[\operatorname{Var}(R^{\mathrm{bank}}) + \operatorname{Var}(\eta)] \in (0,1]$ be the reliability ratio (the signal share of the synthetic proxy).

**(a) Using the actual slice return $R^{\mathrm{bank}}_t$ in Stage 1.** The scale is consistent, $\operatorname{plim}\hat\beta = w$, so each bank-slice-implied loading is correctly scaled:

$$
\text{loading}^{\text{actual}}_k = \hat\beta\,\frac{S_{k,t-1}}{A^{\mathrm{bank}}_{t-1}} + \hat\delta_k, \qquad \operatorname{plim}\hat\beta = w.
\tag{11}
$$

Because $R^{\mathrm{bank}}_t = \widehat{R}^{\,\mathrm{bank}}_t + \eta_t$ *contains* $\eta_t$ while the synthetic factor pieces do not, $\eta_t$ must be carried as an explicit residual line; the decomposition is then factor-explained (correctly scaled) plus a genuine, visible specific/gamma/carry bucket.

**(b) Using the synthetic slice return $\widehat{R}^{\,\mathrm{bank}}_t$ in Stage 1.** The errors-in-variables attenuation gives $\operatorname{plim}\hat\beta = w\,\kappa < w$, so *every* bank-slice-implied loading is uniformly shrunk by the same factor $\kappa$:

$$
\text{loading}^{\text{synth}}_k = \hat\beta\,\frac{S_{k,t-1}}{A^{\mathrm{bank}}_{t-1}} + \hat\delta_k^{\,\prime}, \qquad \operatorname{plim}\hat\beta = w\,\kappa.
\tag{12}
$$

There is no $\eta_t$ to book (it was excluded from the regressor), so the specific bucket is empty *by construction*, not because the fund lacks specific risk. The lost exposure does not vanish; it is reabsorbed into the residual factor loadings and the intercept. In the multivariate model this is the smearing result: writing $\sigma^2_{\widehat{R}^{\,\mathrm{bank}}\mid\mathbf{F}}$ for the variance of the synthetic slice return after partialling out $\mathbf{F}_t$,

$$
\operatorname{plim}\hat\beta = w \cdot \frac{\sigma^2_{\widehat{R}^{\,\mathrm{bank}}\mid\mathbf{F}}}{\sigma^2_{\widehat{R}^{\,\mathrm{bank}}\mid\mathbf{F}} + \operatorname{Var}(\eta)}, \qquad \operatorname{plim}\hat{\boldsymbol{\delta}}^{\,\prime} = \boldsymbol{\delta} + \underbrace{(w - \operatorname{plim}\hat\beta)\,\boldsymbol{\pi}}_{\text{absorbed slice exposure}},
\tag{13}
$$

where $\boldsymbol{\pi} \in \mathbb{R}^{K}$ are the coefficients of projecting the bank-slice factor pieces onto $\mathbf{F}_t$. The attenuated portion of the slice's true exposure, $(w - \operatorname{plim}\hat\beta)$, is thus pushed into $\hat{\boldsymbol{\delta}}^{\,\prime}$ (and any unprojected remainder into $\hat\alpha$), contaminating exactly the residual-exposure and alpha terms one wants to interpret.

**Net effect.**

| Aspect | Actual $R^{\mathrm{bank}}_t$ | Synthetic $\widehat{R}^{\,\mathrm{bank}}_t$ |
|---|---|---|
...
| Slice non-factor P&L $\eta_t$ | explicit residual bucket | absent; leaks into $\hat{\boldsymbol{\delta}}$, $\hat\alpha$ |
| Multivariate side effect | none | $\hat{\boldsymbol{\delta}}$, $\hat\alpha$ contaminated |
| Net on decomposition | clean attribution, honest residual | factor attribution understated, alpha overstated |

In one line: *the actual slice return attributes more of the fund's return to correctly-scaled factor exposure and isolates a true specific/gamma/carry residual; the synthetic slice return shrinks the factor attribution toward zero and pushes the lost exposure into the alpha and residual-factor terms, overstating apparent skill.* Hence use $R^{\mathrm{bank}}_t$ for Stage 1 whenever available, reserving $\widehat{R}^{\,\mathrm{bank}}_t$ for the per-factor split (§6) and the span check (Stage 0).
